...
// ---------------- balance / finance helpers ----------------
async function fetchFinanceOpsForDay({ clientId, apiKey, dateStr }) {
  ...
  while (true) {
    const body = {
      filter: { date: { from: since, to }, operation_type: [] },
      page,
      page_size: pageSize,
    };
    ...
    if (Array.isArray(res.operations)) all.push(...res.operations);

    const hasNext = Boolean(res.has_next);
    if (!hasNext) break;

    page += 1;
    if (page > 20) break; // защита от бесконечного запроса
  }
...
function normalizeOperation(op) {
  if (!op) return null;
  const amountCents = toCents(op.amount || op.amount_rub || op.operation_amount || op.value);

  const postingNumber = op.posting?.posting_number || op.posting?.posting_number_warehouse || op.operation_posting || null;
  const title = op.operation_name || op.type || op.operation_type || "Операция";
  const subtitleParts = [];
  if (op.operation_id) subtitleParts.push(`#${op.operation_id}`);
  if (op.operation_type) subtitleParts.push(String(op.operation_type));
  const subtitle = subtitleParts.join(" · ");

  const occurred_at = op.operation_date || op.operation_date_time || op.date || op.created_at || null;

  const clickable = String(title).toLowerCase().includes("доставка покупателю") && Boolean(postingNumber);

  return { title, subtitle, amount_cents: amountCents, occurred_at, posting_number: postingNumber, clickable };
}

async function handleBalanceOpsToday(req, res) {
  try {
    const resolved = resolveCredsFromRequest(req);
    if (!resolved) return res.status(400).json({ error: "no_creds" });

    const dateStr = todayDateStr();
    const ops = await fetchFinanceOpsForDay({ clientId: resolved.clientId, apiKey: resolved.apiKey, dateStr });
    const normalized = ops.map(normalizeOperation).filter(Boolean);
    normalized.sort((a, b) => {
      const ta = Date.parse(b?.occurred_at || "") || 0;
      const tb = Date.parse(a?.occurred_at || "") || 0;
      return ta - tb;
    });

    return res.json({
      title: `Операции за ${dateStr}`,
      date: dateStr,
      ops: normalized,
    });
  } catch (e) {
...
function serviceName(key) {
  const map = {
    marketplace_service_item_fulfillment: "Фулфилмент",
    marketplace_service_item_pickup: "Забор со склада",
    marketplace_service_item_dropoff_pvz: "Drop-off ПВЗ",
    marketplace_service_item_dropoff_sc: "Drop-off СЦ",
    marketplace_service_item_dropoff_ff: "Drop-off ФФ",
    marketplace_service_item_direct_flow_trans: "Доставка покупателю",
    marketplace_service_item_return_flow_trans: "Возврат от покупателя",
    marketplace_service_item_deliv_to_customer: "Доставка покупателю",
    marketplace_service_item_return_not_deliv_to_customer: "Возврат (не доставлено)",
    marketplace_service_item_return_part_goods_customer: "Возврат (часть заказа)",
    marketplace_service_item_return_after_deliv_to_customer: "Возврат после доставки",
  };
  return map[key] || key;
}

function buildSaleDetail(posting) {
  ...
  const products = Array.isArray(fin.products) ? fin.products : [];

  const saleAmount = products.reduce((acc, p) => {
    const qty = qtyBySku.get(String(p.product_id)) ?? 1;
    return acc + toCents(p.price) * qty;
  }, 0);

  const commissionAmount = products.reduce((acc, p) => {
    const qty = qtyBySku.get(String(p.product_id)) ?? 1;
    return acc + toCents(p.commission_amount) * qty;
  }, 0);

  const payoutAmount = products.reduce((acc, p) => {
    const qty = qtyBySku.get(String(p.product_id)) ?? 1;
    return acc + toCents(p.payout) * qty;
  }, 0);

  const commissionPercent = (() => {
    const percents = products.map((p) => Number(p.commission_percent)).filter((x) => Number.isFinite(x) && x > 0);
    if (!percents.length || saleAmount === 0) return null;
    return Math.round((percents.reduce((a, b) => a + b, 0) / percents.length) * 100) / 100;
  })();

  const serviceObj = fin.posting_services || fin.services || {};
  let servicesAmount = 0;
  const serviceLines = [];
  for (const [key, val] of Object.entries(serviceObj)) {
    const price = typeof val === "number" ? val : val?.price;
    if (price === undefined || price === null) continue;
    const cents = toCents(price);
    if (cents === 0) continue;
    servicesAmount += cents;
    serviceLines.push({ title: serviceName(key), amount_cents: -Math.abs(cents) });
  }

  const expectedPayout = saleAmount - commissionAmount - Math.abs(servicesAmount);
  const otherAdjustments = expectedPayout - payoutAmount;

  const lines = [
    { title: "Продажа", amount_cents: saleAmount },
    { title: "Комиссия", amount_cents: -commissionAmount, percent: commissionPercent },
    ...serviceLines,
  ];

  if (Math.abs(otherAdjustments) > 0) {
    lines.push({ title: "Прочие удержания", amount_cents: otherAdjustments > 0 ? -Math.abs(otherAdjustments) : otherAdjustments });
  }

  return {
    lines,
    note: { title: "К перечислению", amount_cents: payoutAmount },
  };
}

async function handleSaleDetail(req, res) {
  try {
    const resolved = resolveCredsFromRequest(req);
    if (!resolved) return res.status(400).json({ error: "no_creds" });

    const postingNumber = req.query.posting_number || req.query.postingNumber;
    if (!postingNumber) return res.status(400).json({ error: "posting_required" });

    const body = { posting_number: String(postingNumber), with: { financial_data: true } };
    const data = await ozonPost("/v3/posting/fbo/get", { clientId: resolved.clientId, apiKey: resolved.apiKey, body });
    const posting = data?.result || data?.posting || data;

    const detail = buildSaleDetail(posting);
...
app.get("/api/balance/ops/today", handleBalanceOpsToday);
app.get("/api/balance/sale/detail", handleSaleDetail);
